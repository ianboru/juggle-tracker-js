...
class App extends Component {
  ...
  trackBall=(src,colorNum)=>{
    ...
    for (let i = 0; i < contours.length; ++i){
      const circle = cv.minEnclosingCircle(contours[i])

      // If there is no element is position colorNum
      if(!allPositions[colorNum]){
        // Change that element number to a blank list
    ...
    // If there are sorted contours
    if(sortedContourIndices.length > 0){


      // Iterate through the contours
      for(let i = 0; i < Math.min(sortedContourIndices.length, 420); ++i){

        // Get the contour center and size
        ...
        allPositions[colorNum][i]['x'].push(circle.center.x)
        allPositions[colorNum][i]['y'].push(circle.center.y)
        allPositions[colorNum][i]['r'].push(15)
      }
  ...
  colorFilter=(src)=>{
    ...
    this.state.allColors.forEach((colorRange,colorNum)=>{
      if(colorNum > 0){
        ...
        // The color green: 50,57,155,86,255,255
        let lower = [43, 60, 40,0];
        let higher = [79, 186, 255,255];

        // I can't get the trackbars to work. I can't figure out how to log either.
